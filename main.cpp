...
void BoyerMoore_PerformanceTest()
{
    UNICODE_STRING usFullPath, usSearchName;
    StopWatch sw;
    LONG IndexOf = 0, Sum, n, i;
    ...
    for (n = 0; n < sizeof(szSearchName) / sizeof(WCHAR *); ++n) {
        ...
        BoyerMoore_Preprocessing(usSearchName.Buffer, usSearchName.Length / sizeof(WCHAR));

        Sum = 0;
        sw.start();
        for (i = 0; i < Iteration; ++i) {
            index = BoyerMoore(usFullPath.Buffer, usFullPath.Length / sizeof(WCHAR),
                usSearchName.Buffer, usSearchName.Length / sizeof(WCHAR), &IndexOf);
            noif(NT_SUCCESS(Status) || (Status == STATUS_NOT_FOUND)) {
                Sum += IndexOf;
            }
        }
